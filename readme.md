...
-   **클라이언트 (Frontend)**: `client` 디렉토리에 있으며, [React]를 사용하여 구축되었습니다. 사용자가 포트폴리오를 구성하고 백테스트를 실행하며 그 결과를 확인할 수 있는 웹 인터페이스(UI)를 제공합니다.
...
-   **데이터베이스**: [MongoDB]를 사용하여 데이터를 영구적으로 저장합니다. 주로 사용자가 생성한 포트폴리오와 외부 API에서 가져온 주식 데이터를 캐싱하는 데 사용됩니다.
-   **외부 서비스**: [Yahoo Finance API](https://finance.yahoo.com/)를 통해 실제 주식 시장의 과거 데이터를 가져옵니다. `yahoo-finance2` 라이브러리를 사용하여 통신합니다.

## 2. 백엔드 (Server) 상세 설명

서버는 API 제공, 비즈니스 로직 처리, 데이터 관리를 담당합니다.

-   **`server.js`**: 서버의 메인 진입점입니다. Express 앱을 설정하고, MongoDB 데이터베이스에 연결하며, API 라우터들을 등록합니다.

-   **`routes/`**: API 엔드포인트를 정의합니다.
    -   `portfolios.js`: 포트폴리오에 대한 CRUD(생성, 조회, 수정, 삭제) API를 제공합니다.
    -   `stocks.js`: 주식 종목을 검색하고 특정 종목의 과거 데이터를 조회하는 API를 제공합니다.
...
-   **`services/`**: 핵심 비즈니스 로직을 포함합니다.
    ...
    -   `backtestService.js`: 실제 백테스트 시뮬레이션을 수행합니다. `stockService`를 통해 필요한 모든 주식 데이터를 가져온 후, "Buy and Hold" 투자 전략을 시뮬레이션하고, 총수익률, 연환산수익률, 최대 낙폭(MDD), 샤프 지수 등 다양한 성과 지표를 계산합니다.

-   **`models/`**: MongoDB 데이터베이스의 스키마를 정의합니다.
    -   `Portfolio.js`: 사용자가 생성한 포트폴리오의 구조(이름, 보유 종목 및 비중 등)를 정의합니다.
    -   `Stock.js`: `stockService`가 캐싱할 주식 데이터의 구조를 정의합니다.
    -   `BacktestResult.js`: 백테스트 실행 결과를 저장하기 위한 스키마입니다.

## 3. 프론트엔드 (Client) 상세 설명

클라이언트는 사용자 경험(UX)을 만들고 서버와 상호작용합니다.

-   **`App.js`**: 애플리케이션의 메인 React 컴포넌트입니다. 전체 UI의 흐름을 단계별 마법사(Wizard) 형태로 관리합니다.
    1.  포트폴리오 이름/초기 자본 설정
...
-   **`components/`**: 재사용 가능한 UI 조각들입니다.
    -   `PortfolioBuilder.jsx`: 포트폴리오를 생성하고 편집하는 UI 컴포넌트입니다.
    -   `PortfolioList.jsx`: 저장된 포트폴리오 목록을 보여주는 컴포넌트입니다.
    -   `BacktestResults.jsx`: 백테스트 결과를 시각적으로 표현하는 컴포넌트입니다.
-   **`services/api.js`**: 서버 API와의 모든 통신을 중앙에서 관리하는 모듈입니다. `axios` 라이브러리를 사용하여 백엔드에 HTTP 요청을 보내고 응답을 처리합니다. 이를 통해 컴포넌트 코드와 API 호출 로직을 분리합니다.
