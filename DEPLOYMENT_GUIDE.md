...
이 프로젝트는 정적웹사이트인가? 동적웹사이트인가?, 클라이언트사이드 렌더링인지?

✦ 이 프로젝트의 구조를 바탕으로 답변해 드리겠습니다.

  결론부터 말씀드리면, 이 프로젝트는 동적 웹사이트(Dynamic Website)이며, 렌더링 방식은
  클라이언트 사이드 렌더링(Client-Side Rendering, CSR)입니다.

  ---

  1. 동적 웹사이트 vs 정적 웹사이트

  이 프로젝트는 동적 웹사이트입니다. 그 이유는 다음과 같습니다.

   * 독립된 백엔드 서버(`server`)의 존재: 프로젝트 내에 server 디렉터리가 있으며, 이는
     Node.js와 Express를 사용하여 API 서버의 역할을 합니다. 이 서버는 routes, services,
     models를 통해 데이터베이스와 상호작용하거나, 비즈니스 로직(ex: 주식 데이터 조회,
     백테스팅 계산)을 수행하여 그 결과를 동적으로 생성하고 제공합니다.
   * 데이터 통신: client(프론트엔드)는 server(백엔드)에 API를 요청하여 데이터를
     받아옵니다. 예를 들어, 사용자가 포트폴리오 조회를 요청하면 클라이언트는 백엔드에
     데이터를 요청하고, 백엔드는 데이터베이스에서 해당 정보를 찾아 응답합니다. 이처럼
     서버로부터 실시간으로 데이터를 받아와 화면을 구성하므로 동적 웹사이트입니다.

  (만약 HTML, CSS, JS 파일만으로 구성되어 서버와의 데이터 통신 없이, 빌드된 시점의
  내용만 보여준다면 정적 웹사이트입니다.)

  2. 클라이언트 사이드 렌더링 (CSR)

  이 프로젝트의 프론트엔드는 클라이언트 사이드 렌더링(CSR) 방식으로 동작합니다.

   * React 기반의 SPA(Single Page Application): client 디렉터리는 create-react-app으로
     구성된 전형적인 SPA 구조입니다.
   * 렌더링 과정:
       1. 사용자가 웹사이트에 접속하면, 서버는 거의 비어있는 public/index.html 파일과
          자바스크립트(React) 파일을 보냅니다.
       2. 브라우저는 이 자바스크립트 파일을 다운로드하여 실행합니다.
       3. 자바스크립트(React)가 API 서버(server)에 필요한 데이터를 요청합니다.
       4. 데이터를 받아온 후, React가 동적으로 DOM을 생성하고 조작하여 사용자에게
          보여줄 화면을 브라우저(클라이언트)에서 직접 그립니다.

  이처럼 대부분의 렌더링 책임이 서버가 아닌 클라이언트(브라우저)에 있기 때문에
